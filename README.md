...
| — | `select_marked` | Call `on_select` with all marked entries as a list |

### Multiselect

Pass `multiselect = true` to `pick()` to choose several entries at once:
...
refer.pick(
    ...
    function(selection)
        -- in multiselect mode, selection is always a list
        print("Picked: " .. table.concat(selection, ", "))
    end,
    { multiselect = true }
)
```

- `<Tab>` toggles a mark and moves down; `<CR>` delivers all marked entries as
  a list.
- Marks survive filtering.
- These bindings are per-picker defaults - your own keymaps for the same keys
  win.
